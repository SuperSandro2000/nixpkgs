...
let
  ...
  # base packages of the bubblewrap
  ...
  etcProfile = writeText "profile" ''
    export PS1='${name}-bwrapenv:\u@\h:\w\$ '
  ...
  # Compose /etc for the bubblewrap environment
  etcPkg = runCommandLocal "${name}-bwrap-etc" { } ''
    mkdir -p $out/etc
    pushd $out/etc
  ...
  # the target profile is the actual profile that will be used for the bubblewrap
  setupTargetProfile = ''
    mkdir -m0755 usr
    pushd usr
