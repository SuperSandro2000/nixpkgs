...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.postgresqlBackup.databases = lib.mkIf (cfg.backupAllExcept != [ ]) (lib.filter (db: lib.any (x: db != x) cfg.backupAllExcept) config.services.postgresql.databases);
    ...
    systemd.services = lib.mkMerge [
      ...
      (lib.mkIf (!cfg.backupAll) (lib.listToAttrs (
        map (
          db:
          let
