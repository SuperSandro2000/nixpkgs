...
let
  ...
  moduleOpts = {
    ...
    csi_simple = mkOption {
      type = types.bool;
      default = false;
      description = lib.mdDoc "Implements simple traffic optimisation for clients that have reported themselves as inactive to mod_csi";
    };
...

{
  ...
  config = mkIf cfg.enable {
    ...
    environment.etc."prosody/prosody.cfg.lua".source = pkgs.runCommandLocal "prosody.cfg.lua-checked" {
    ...
    systemd.services.prosody = {
      ...
      preStart = ''
        ${pkgs.envsubst}/bin/envsubst -i ${configFile} -o /run/prosody/prosody.cfg.lua
      '';
